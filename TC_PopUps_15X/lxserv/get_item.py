...
class Popupfifteen(QDialog):
    ...
    def process_selection(self):
        ...
        if index.isValid():
            ...
            try:
                lx.eval("popupfifteen.createItem {%s}" %item)
...
class GetItem ( lxu.command.BasicCommand ):
    '''
    Custom Command to spawn the popupfifteen.
    '''
    def __init__(self):
        lxu.command.BasicCommand.__init__(self)
    ...
    def basic_Execute(self, msg, flags):
        ...
        '''
        self.popupfifteen = Popupfifteen(self.dyna_String(0))

        # Move the dialog to the cursor's position
        self.popupfifteen.move( QCursor().pos() )

        # Using exec_() causes instabilities, so we'll use show() instead
        # and set the dialog as modal in its constructor.
        self.popupfifteen.show()


# Bless this mess!
parse_all_the_things()
lx.bless(CreateItem, "popupfifteen.createItem")
lx.bless(GetItem, "popupfifteen.getItem")
